    ...
    */
    ...
    if (request.body.type === 'event_callback' && !parsedMessage.originalRequest.event.bot_id) {
      ...
      return bot(parsedMessage, request)
        .then(botResponse => slackReply(parsedMessage.originalRequest.event.channel, botResponse, request.env.slackOauthToken))
        .then(() => 'OK')
        .catch(logError);
    }
  ...
  api.addPostDeployStep('slackSlashCommand', (options, lambdaDetails, utils) => {
    return Promise.resolve().then(() => {
      ...
      if (options['configure-slack-slash-app']) {
        ...
        return prompt(['Slack Client ID', 'Slack Client Secret', 'Slack verification token', 'Slack OAuth Access Token', 'Home page URL'])
          .then(results => {
            const deployment = {
              restApiId: lambdaDetails.apiId,
              stageName: lambdaDetails.alias,
