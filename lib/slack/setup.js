...
module.exports = function slackSetup(api, bot, logError, optionalParser, optionalResponder) {
  ...
  api.post('/slack/events', request => {
    ...
    */
    ...
    const parsedMessage = parser(request.body);

    if (request.body.type === 'event_callback' && !parsedMessage.originalRequest.event.bot_id) {
      const channelId = parsedMessage.originalRequest.event.channel;
      const tokenPromise = db.getSlackToken(channelId);
      const botPromise = bot(parsedMessage, request);

      if (parsedMessage.originalRequest.event.type === 'tokens_revoked') {
        const botId = parsedMessage.originalRequest.event.tokens.bot[0];
        return db.revokeSlackToken(botId)
            .catch(logError);
      }

      return Promise.all([botPromise, tokenPromise])
        .then(values => slackReply(channelId, values[0], values[1]))
        .then(() => 'OK')
        .catch(logError);
    }
  ...
  api.get('/slack/landing', request => {
    return rp.post('https://slack.com/api/oauth.access', {
      ...
      .then((res) => {
        ...
        if (body.ok) {
          const bot_access_token = body.bot.bot_access_token;
          const bot_user_id = body.bot.bot_user_id;
          ...
          return rp.post('https://slack.com/api/conversations.open', {
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded'
            },
            ...
            if (body.ok) {
              ...
              return db.storeSlackToken(channel_id, bot_user_id, bot_access_token);
            } else {
          ...
          logError(body);
          return request.env.slackHomePageUrl;
        }
      });
  }, {
    success: 302
  ...
  api.addPostDeployStep('slackSlashCommand', (options, lambdaDetails, utils) => {
    return Promise.resolve().then(() => {
      ...
      if (options['configure-slack-slash-app']) {
        ...
        console.log(`\nAnd if you need to subscribe to Slack events, your event Request URL is ${color.cyan}${lambdaDetails.apiUrl}/slack/events${color.reset}\n`);
        ...
        return prompt([
          ...
          {
            question: 'Slack OAuth Access Token(optional, for Event API)',
            optional: true,
            default: ''
          },
          'Home page URL'])
          .then(results => {
            const deployment = {
              ...
              variables: {
                ...
                slackOauthToken: results['Slack OAuth Access Token'],
      ...
      .then(() => [
        `${lambdaDetails.apiUrl}/slack/slash-command`,
        `${lambdaDetails.apiUrl}/slack/message-action`,
        `${lambdaDetails.apiUrl}/slack/events`,
        `${lambdaDetails.apiUrl}/slack/landing`
      ]);
  });
};
