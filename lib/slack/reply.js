...
module.exports = function(channelId, message, token) {
  ...
  if (typeof message === 'string') {
    ...
    // attachments should be a JSON-encoded string.
    if (message.attachments)
      message.attachments = encodeURIComponent(JSON.stringify(message.attachments));
  ...
  return rp.post('https://slack.com/api/chat.postMessage', {
    ...
    // encodeURIComponent function next line avoids attachments to be encoded again !
    body: qs.stringify(params, null, null, { encodeURIComponent: str => str })
  });
};
